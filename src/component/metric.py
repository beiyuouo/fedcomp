...
def WatershedCrossEntropy(input, target):
    ...
    return torch.mean(disc_DT * CEloss[:, 0, :, :] + cup_DT * CEloss[:, 1, :, :])
...
def dice_coeff_2label(pred, target):
    ...
    # print pred.shape
    if len(pred.shape) == 3:
        return dice_coefficient_numpy(pred[0, ...], target[0, ...]), dice_coefficient_numpy(
            pred[1, ...], target[1, ...])
    else:
        dice_cup = []
        dice_disc = []
        for i in range(pred.shape[0]):
            cup, disc = dice_coefficient_numpy(pred[i, 0, ...],
                                               target[i, 0, ...]), dice_coefficient_numpy(
                                                   pred[i, 1, ...], target[i, 1, ...])
            dice_cup.append(cup)
