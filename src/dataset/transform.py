# Code Reference: https://github.com/emma-sjwang/Dofe/blob/master/dataloaders/custom_transforms.py
...
class elastic_transform():
    ...
    def __call__(self, sample):
        ...
        if seed > 0.5:
            ...
            dx = gaussian_filter(
                (random_state.rand(*shape) * 2 - 1), sigma, mode="constant", cval=0) * alpha
            dy = gaussian_filter(
                (random_state.rand(*shape) * 2 - 1), sigma, mode="constant", cval=0) * alpha
            ...
            for i in range(3):
                # print(i)
                transformed_image[:, :, i] = map_coordinates(np.array(image)[:, :, i],
                                                             indices,
                                                             order=1).reshape(shape)
                # break
            if label is not None:
                transformed_label[:, :] = map_coordinates(np.array(label)[:, :],
                                                          indices,
                                                          order=1,
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if self.padding > 0 or w < self.size[0] or h < self.size[1]:
            padding = np.maximum(
                self.padding,
                np.maximum((self.size[0] - w) // 2 + 5, (self.size[1] - h) // 2 + 5))
...
class FixedResize(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': img, 'label': mask, 'img_name': name}
